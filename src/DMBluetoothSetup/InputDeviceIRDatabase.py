    ...
    def reload(self):
        try:
            if pathExists(self.IRDB):
                with open(self.IRDB, "r", encoding="utf-8") as irdb_file:
                    json_data = irdb_file.read()
                if json_data:
                    self._irdb = OrderedDict(sorted(json.loads(json_data).items()))
                    return
