...
class DfuWelcomeStep(SetupTextStep):
    ...
    def prepare(self):
        ...
        self.text = _(
            "to the Remote Control Firmware Update Assistant.\n"
...
class DfuUpdateStep(SetupListStep, InputDeviceManagementBase):
    ...
    def buttons(self):
        ...
        if isinstance(device, eManagedInputDevicePtr) and (device.connected() or device.isDfu()):
            ...
            text = _(
                "Press Blue to update the firmware of the selected remote control "
                "to application version {0} and bootloader version {1}"
            ).format(".".join(str(x) for x in version), self.getBootloaderVersion())
        else:
        ...
        self.parent.text = "{0}\n\n{1}".format(
            text,
            _("Press OK to finish the Remote Control Firmware Update Assistant."),
        )
